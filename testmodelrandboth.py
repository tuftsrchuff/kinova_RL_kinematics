...
save_freq = 5000
model_per_rand = 20
f = open(f"./rewards/rand_both_rewards.txt", "w")
for model_num in range(1,model_per_rand+1):
    mean_reward_all_mod = 0
    for i in range(1, models+1):
        model_string = f"./logs/Rand_both_model_mod_{i}_{save_freq*model_num}_steps.zip"
        print(f"Loading model {model_string}")
        model = PPO.load(model_string, env=env)
        print("Evaluating policy")
        mean_reward, std_reward = evaluate_policy(model, env, n_eval_episodes=25, deterministic=True, render=False, callback=None, reward_threshold=None, return_episode_rewards=False, warn=True)
    ...
    print(f"Writing to file: {mean_reward_all_mod}")
